...
describe('#rpnCalculator', () => {
  ...
  it('if another operator has been passed, a message error should be returned', () => {
    expect(rpnCalculator('3 4 .')).toThrows('You should only use the following operators: +, -, *, /');
    expect(rpnCalculator('1 2 3 . ^')).toThrows('You should only use the following operators: +, -, *, /');
  });
});
